...
def chebqkud(
    ...
    *,
    ...
    The basis vectors are generated as Chebyshev-polynomial vectors:
    ...
    The returned ``basis_states`` are overlap-orthonormalized combinations of
    the generated Chebyshev vectors. Numerically dependent directions with
    overlap eigenvalues below ``overlap_tol`` are discarded, so the returned
    basis can contain fewer than ``n_steps + 1`` vectors.
    ...
    shots_value = None if shots is None else int(shots)
    shot_rng = np.random.default_rng(shot_seed) if shots_value is not None else None
    ...
    def _sample_bounded_expectation(value, scale):
        if shots_value is None:
            return value
        if scale <= 0.0:
            return 0.0

        mean = float(np.clip(value / scale, -1.0, 1.0))
        probability = 0.5 * (mean + 1.0)
        samples = shot_rng.binomial(shots_value, probability)
        sampled_mean = (2.0 * samples / shots_value) - 1.0
        return scale * sampled_mean

    def _apply_hamiltonian_shot_noise(matrix, scale):
        if shots_value is None:
            return matrix

        noisy = np.array(matrix, dtype=complex, copy=True)
        dim = noisy.shape[0]
        for i in range(dim):
            noisy[i, i] = _sample_bounded_expectation(noisy[i, i].real, scale)
            for j in range(i + 1, dim):
                real = _sample_bounded_expectation(noisy[i, j].real, scale)
                imag = _sample_bounded_expectation(noisy[i, j].imag, scale)
                noisy[i, j] = real + 1j * imag
                noisy[j, i] = real - 1j * imag

        return 0.5 * (noisy + noisy.conj().T)
    ...
    def _projected_problem(current_basis_states):
        ...
        orthonormal_basis = X.T @ current_basis_states
        S_ortho = orthonormal_basis.conj() @ orthonormal_basis.T
        S_ortho = 0.5 * (S_ortho + S_ortho.conj().T)

        H_ortho = X.conj().T @ H_k @ X
        H_ortho = 0.5 * (H_ortho + H_ortho.conj().T)
        H_ortho = _apply_hamiltonian_shot_noise(H_ortho, hamiltonian_shot_scale)
        energies = np.linalg.eigvalsh(H_ortho).real
        return energies, S_ortho, H_ortho, orthonormal_basis
    ...
    h_min_for_noise, h_max_for_noise = _estimate_energy_bounds(H_mat)
    hamiltonian_shot_scale = max(
        abs(h_min_for_noise),
        abs(h_max_for_noise),
        1.0,
    )
    ...
    raw_basis_states = np.stack(basis_states, axis=0)
    energies, S, H_k, basis_states = _projected_problem(raw_basis_states)
    ...
    if return_min_energy_history:
        min_energy_history = []
        for k in range(1, raw_basis_states.shape[0] + 1):
            if k == raw_basis_states.shape[0]:
                min_energy_history.append(float(energies[0]))
            else:
                e_k, _, _, _ = _projected_problem(raw_basis_states[:k])
